...
def valid_passwords_part_one(_max, _min, letter, password):
    count = 0
    for l in password:
        if l == letter:
            count += 1
    return _max >= count >= _min

def valid_password_part_two(_max, _min, letter, password):
    return (password[_max - 1] == letter) ^ (password[_min - 1] == letter)

valid = 0
valid_b =0
for rule in _input:
    _rule = rule.split(':')
    password = _rule[1].strip()
    rule_letter = _rule[0].split(' ')
    letter = rule_letter[1].strip()
    _min, _max = map(int, rule_letter[0].split('-'))
    if valid_passwords_part_one(_max, _min, letter, password):
        valid += 1
    if valid_password_part_two(_max, _min, letter, password):
        valid_b += 1
